...
public class EditMenuItemServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		try {
			...
			MenuItem menuItem1 = new MenuItem(Long.parseLong(id), name,
					(float) price, active, d, category, freeDelivery);
			...
			request.getRequestDispatcher("edit-menu-item-status.jsp").forward(
					request, response);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
